...
pathFinder createCommands(pathFinder coord_list) // crée une suite d'instructions pour le robot en fonction d'une suite de cases à visiter
{
    ...
    {
        if (!((coord_list.path_coord[i].i == coord_list.path_coord[i - 1].i) && (coord_list.path_coord[i].i == coord_list.path_coord[i + 1].i) || (coord_list.path_coord[i].j == coord_list.path_coord[i - 1].j) && (coord_list.path_coord[i].j == coord_list.path_coord[i + 1].j)))
        {
    ...
    //     else
    //     {
    //         while (coord_list.path_coord[c].j == coord_list.path_coord[c + k].j)
    //         {
    //             k++;
    //         }
    //     }
